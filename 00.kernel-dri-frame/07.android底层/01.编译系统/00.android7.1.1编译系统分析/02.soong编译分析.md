...
ifeq ($(USE_SOONG),true)                                                                     
checkbuild: checkbuild-soong # 如果开启的话，　会执行 checkbuild-soong 目标
endif
ifeq (true,$(ANDROID_BUILD_EVERYTHING_BY_DEFAULT))
droid: checkbuild　# 从这里我们知道，如果不开启soong编译的话，　make 默认会执行　checkbuild 目标
endif
```
...
```shell
make iso_img 'USE_SOONG=true' # 此时就会会　开启 soong 编译，也就是开启用 Android.bp　开始编译　而不用 Android.mk进行编译
```
