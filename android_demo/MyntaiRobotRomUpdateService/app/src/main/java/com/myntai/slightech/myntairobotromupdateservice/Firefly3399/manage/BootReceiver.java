package com.myntai.slightech.myntairobotromupdateservice.Firefly3399.manage;
...
public class BootReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        AndroidOtaLogTree.log("OTA boot receive.");
        if (OTA_BOOT_URL.equals(intent.getAction())) {
            ...
            if (null == OtaPackageFilePath) {
                AndroidOtaLogTree.log("last_install is null ,check PackageGoUp?");
            ...
            if (upgradeStatus.equals("1") && deleteStatus.equals(DELETE_SUCCESS)) {
                // 此时状态：升级成功　并且　删除成功
                AndroidOtaLogTree.log("check packageGoup?");
                checkContinueGoUpRomAndHandle();
               ...
               // "onReceive: 此时弹窗提示用户　升级成功　是否删除升级包");
                ...
                AndroidOtaLogTree.log("last up go fail.");
                ...
                try {
                    ...
                    AndroidOtaLogTree.log(getClass().getSimpleName(), "onReceive()", "rm " + dir + "*");
                }
            }else if(upgradeStatus.equals("0") && deleteStatus.equals(DELETE_SUCCESS)){
                AndroidOtaLogTree.log("check packageGoup?");
                checkContinueGoUpRomAndHandle();
    ...
    private void checkContinueGoUpRomAndHandle() {
        try {
            ...
            if (null == isContinueUpGo) {
                ...
                AndroidOtaLogTree.log("Upgrade is needed at this time.");
                //发送循环升级广播
                sendPollGoUpBroadcast();
            }
        } catch (Exception e) {
            AndroidOtaLogTree.log(getClass().getSimpleName(),"checkContinueGoUpRomAndHandle()","" + e);
        }
    }
    ...
    private void checkAndDeleteTempOtaLogFirstRow() {
        try {
            ...
            if (lastInstallFileInUpdateFile.equals(tempOtaLogInUpdateFile)) {
                ...
                AndroidOtaLogTree.log("temp_ota_log first line delete success.");
            } else {
                //不删除 循环升级时，再次升级
            }

        } catch (Exception e) {
            AndroidOtaLogTree.log(getClass().getSimpleName(),"checkAndDeleteTempOtaLogFirstRow()","" + e);
        }
    }
