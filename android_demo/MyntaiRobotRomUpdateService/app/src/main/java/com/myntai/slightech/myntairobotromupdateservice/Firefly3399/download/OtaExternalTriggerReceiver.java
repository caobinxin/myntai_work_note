...
public class OtaExternalTriggerReceiver extends BroadcastReceiver implements ICheckServer, Handler.Callback, IDownloadStatus {
    ...
    public void onDownLoadAllAUTOSuccess(Object object) {
        AndroidOtaLogTree.log("自动升级　全量包　已经下载完毕 通知升级模块直接全量包进行升级");
    ...
    public void onDownLoadAllSuccess(Object object) {
        ...
        AndroidOtaLogTree.log("全包下载成功　需要升级:　" + allPkgName);
    ...
    public void onDownLoadDiffSuccess(Object object) {
        ...
        AndroidOtaLogTree.log("差分包下载成功　需要升级:　" + count);
    ...
    public void onUpgrade(Object object) {
        ...
        DownloadPkg.getInstance().set3399AppUpdate(mAppupdate);
        DownloadPkg.getInstance().set3399Listen(this);
        ...
        if(mMode == Common.MODE_AUTOMATIC){
            ...
            AndroidOtaLogTree.log("ota　外界触发方式不对，是全自动还是手动?请指定，否则不触发升级");
        }
    ...
    public void onNoUpgrade(Object object) {
        ...
        AndroidOtaLogTree.log("当前就是最新版本，不需要更新.");
        Message message = Message.obtain();
    ...
    public void onReceive(Context context, Intent intent) {
        AndroidOtaLogTree.log("OtaExternalTriggerReceiver", "onReceive", "触发全局外部的入口");
        ...
        if (OTA_EXTERNAL_TRIGGER_BROADCAST_URL.equals(intent.getAction())) {
            ...
            AndroidOtaLogTree.log("Mode :" + mMode);
    ...
    private void checkServerAndUpgrades() {
        if (null != checkServer) {
            ...
            AndroidOtaLogTree.log("checkserver == null");
        }
    }
    ...
    private void startTheWindow() {
        ...
        if (SystemDialogUtil.requestAlertWindowPermission()) {
            ...
            AndroidOtaLogTree.log("OtaExternalTriggerReceiver", "startTheWindow", "没有dialog的权限");
        }
    ...
    private void autoDownloadGoup(){
        if(null != mAppupdate){
            ...
            AndroidOtaLogTree.log("OtaExternalTriggerReceiver", "autoDownloadGoup", "null == mAppupdate 无法完成自动升级");
        }
    }
